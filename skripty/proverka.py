...
def otbrosit_shapku_kp(text, path=""):
    ...
    if not lines or not lines[0].lower().startswith(("формат:", "тема:", "статус:")):
        return text
    # Ищем разделитель по всей шапке, а не в первых двенадцати строках.
    # Шапки давно длиннее: тема на абзац плюс пометки продюсеру ⚠️ на
    # каждое решение. При жёстком лимите шапка попадала в текст и портила
    # и метрики, и лимит Telegram, и сверку с корпусом — молча, потому что
    # строка «шапка отброшена» просто не печаталась. Ограничение оставлено
    # щедрым, но не бесконечным: если `---` нет вовсе, текст возвращается
    # как есть, и это по-прежнему обычный текст без шапки.
