...
const tariffTable = new Table({
  ...
  rows: [
    ...
    featRow("Готовые фразы", "Как сказать «нет» и как попросить за себя. Слова, которые можно произнести вслух", [1,1,1]),
    featRow("Конституция эмпата", "Ваши личные права одним листом. То, к чему возвращаются и держат под рукой", [1,1,1]),
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      // ===== МАНИФЕСТ ЦЕННОСТИ =====
      H2("Вы всё время доказываете, что чего-то стоите"),
      P("Работой. Заботой. Тем, что всех выслушали и всем помогли. А ощущение остаётся прежнее: ценят за то, что вы делаете. Не за то, что вы есть."),
      RUNS([
        { t: "Виола отвечает на это прямо. " },
        { t: "Ваша ценность врождённая. Она не результат ваших достижений, ваших ошибок или чужого одобрения.", b: true },
        { t: " Её не нужно заслуживать. Нужно перестать от неё отказываться." },
      ]),
      P("А начинается это с того, что вы наконец увидите, чем именно вы ценны.", { after: 160 }),
      ...
      ...lect("Лекция 1. Куда уходит ваша сила и почему именно к этим людям",
        "ловить момент, когда ваш радар включается на чужое. И находить свой крючок — то место, за которое вас цепляют раз за разом.",
        "видите, кто именно вас обесточивает. И понимаете, почему рядом снова оказался такой человек.",
        "видите, где вы годами работаете за спасибо."),
      ...lect("Лекция 2. Вернуть себе энергию",
        "считывать себя так же точно, как вы читаете других. Ставить границы, не выключая чувствительность. И возвращать телу опору: сон, движение, простые вещи, на которых всё держится.",
      ...
      BULLET("Конституцию эмпата: ваши личные права, к которым можно возвращаться."),
...
BULLET("Сообщить ей, что индивидуальные встречи один на один в тарифы не идут: час её времени не тиражируется и упирает потолок продаж в её календарь. Вместо встречи — два разбора голосом, её формат из «Эмпатии»."),
      ...
      BULLET("Дни недели: середина недели и выходные — какие именно."),
      BULLET("Дата старта первого потока."),
      BULLET("«Конституцию эмпата» — сам документ. Строка уже стоит в тарифах и в «что вы получите», но документа у нас нет: на клиентах не показываем, пока не пришлёт."),
