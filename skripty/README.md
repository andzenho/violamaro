...
- `transkript.py` — видео Виолы (YouTube / Instagram / локальный файл) →
  ...
  `yt-dlp` и JS-движок для YouTube (`deno`, либо уже стоящий `node` — скрипт
  подставит его сам). **`ffmpeg` необязателен:** AssemblyAI принимает видео и
  звук как есть и всё равно приводит к 16 кГц, а `yt-dlp -f bestaudio` отдаёт
  готовый одиночный аудиофайл. Сведение экономит только объём заливки (втрое-
  впятеро) и нужно для `--skorost`; нет ffmpeg — скрипт скажет и отправит как
  есть, принудительно — флаг `--kak-est`. Ключ — `ASSEMBLYAI_API_KEY` или файл `.assemblyai-key`
